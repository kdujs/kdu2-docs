...
<div id="dynamic-component-demo" class="demo">
  <button
    k-for="tab in tabs"
    k-bind:key="tab"
    k-bind:class="['dynamic-component-demo-tab-button', { 'dynamic-component-demo-active': currentTab === tab }]"
    ...
    <div class="dynamic-component-demo-posts-tab">\
      <ul class="dynamic-component-demo-posts-sidebar">\
        <li\
          k-for="post in posts"\
          k-bind:key="post.id"\
          k-bind:class="{ \'dynamic-component-demo-active\': post === selectedPost }"\
          k-on:click="selectedPost = post"\
        >\
          {{ post.title }}\
        </li>\
      </ul>\
      <div class="dynamic-component-demo-post-container">\
        ...
        >\
          ...
          <div k-html="selectedPost.content"></div>\
        </div>\
        <strong k-else>\
...
You'll notice that if you select a post, switch to the _Archive_ tab, then switch back to _Posts_, it's no longer showing the post you selected. That's because each time you switch to a new tab, Kdu creates a new instance of the `currentTabComponent`.
...
Now the _Posts_ tab maintains its state (the selected post) even when it's not rendered. See [this example](https://codesandbox.io/s/github/kdujs/kdu2-docs/tree/master/src/v2/examples/kdu-20-keep-alive-with-dynamic-components) for the complete code.
...
In large applications, we may need to divide the app into smaller chunks and only load a component from the server when it's needed. To make that easier, Kdu allows you to define your component as a factory function that asynchronously resolves your component definition. Kdu will only trigger the factory function when the component needs to be rendered and will cache the result for future re-renders. For example:
...
``` js
Kdu.component('async-webpack-example', function (resolve) {
...
const AsyncComponent = () => ({
  ...
  component: import('./MyComponent.kdu'),
...
> Note that you must use `Kdu Router` 2.4.0+ if you wish to use the above syntax for route components.
