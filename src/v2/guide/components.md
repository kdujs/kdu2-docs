...
Since components are reusable Kdu instances, they accept the same options as `new Kdu`, such as `data`, `computed`, `watch`, `methods`, and lifecycle hooks. The only exceptions are a few root-specific options like `el`.
