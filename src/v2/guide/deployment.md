...
> Most of the tips below are enabled by default if you are using [Kdu CLI](https://kdujs-cli.web.app). This section is only relevant if you are using a custom build setup.
...
If you are using the full build, i.e. directly including Kdu via a script tag without a build tool, make sure to use the minified version (`kdu.min.js`) for production. Both versions can be found in the [Installation guide](installation.html#Direct-lt-script-gt-Include).
...
- Run your bundling command with the actual `NODE_ENV` environment variable set to `"production"`. This tells `kduify` to avoid including hot-reload and development related code.

- Apply a global [envify](https://github.com/hughsk/envify) transform to your bundle. This allows the minifier to strip out all the warnings in Kdu's source code wrapped in env variable conditional blocks. For example:
