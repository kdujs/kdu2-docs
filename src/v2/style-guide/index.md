...
**Component names should always be multi-word, except for root `App` components, and built-in components provided by Kdu, such as `<transition>` or `<component>`.**
...
When using the `data` property on a component (i.e. anywhere except on `new Kdu`), the value must be a function that returns an object.
...
// Kdu instance, since only a single instance
// will ever exist.
new Kdu({
  data: {
    foo: 'bar'
...
### Keyed `k-for` <sup data-p="a">essential</sup>

**Always use `key` with `k-for`.**
...
### Avoid `k-if` with `k-for` <sup data-p="a">essential</sup>
...
When Kdu processes directives, `k-for` has a higher priority than `k-if`, so that this template:
...
- Using `k-for="user in activeUsers"`, we _only_ iterate over active users during render, making rendering much more efficient.
...
By moving the `k-if` to a container element, we're no longer checking `shouldShowUsers` for _every_ user in the list. Instead, we check it once and don't even evaluate the `k-for` if `shouldShowUsers` is false.
...
This is only relevant for [single-file components](../guide/single-file-components.html). It does _not_ require that the `scoped` attribute be used. Scoping could be through CSS modules, a class-based strategy such as [BEM](http://getbem.com/), or another library/convention.
...
``` js
Kdu.component('TodoList', {
  // ...
})

Kdu.component('TodoItem', {
  // ...
})
```
{% raw %}</div>{% endraw %}
...
But they'll **never** contain global state (e.g. from a Kdux store).
...
**Component names in JS/[JSX](../guide/render-function.html#JSX) should always be PascalCase, though they may be kebab-case inside strings for simpler applications that only use global component registration through `Kdu.component`.**
...
However, for applications that use **only** global component definitions via `Kdu.component`, we recommend kebab-case instead. The reasons are:
...
### `k-if`/`k-else-if`/`k-else` without `key` <sup data-p="d">use with caution</sup>

**It's usually best to use `key` with `k-if` + `k-else`, if they are the same element type (e.g. both `<div>` elements).**

By default, Kdu updates the DOM as efficiently as possible. That means when switching between elements of the same type, it simply patches the existing element, rather than removing it and adding a new one in its place. This can have [unintended consequences](https://codesandbox.io/s/github/kdujs/kdu2-docs/tree/master/src/v2/examples/kdu-20-priority-d-rules-unintended-consequences) if these elements should not actually be considered the same.
...
**`Kdux` should be preferred for global state management, instead of `this.$root` or a global event bus.**
...
Kdux is the [official flux-like implementation](/v2/guide/state-management.html#Official-Flux-Like-Implementation) for Kdu, and offers not only a central place to manage state, but also tools for organizing, tracking, and debugging state changes. It integrates well in the Kdu ecosystem (including full [Kdu DevTools](/v2/guide/installation.html#Kdu-Devtools) support).
...
(function () {
  var enforcementTypes = {
    ...
    linter: '<a href="https://github.com/kdujs/eslint-plugin-kdu#eslint-plugin-kdu" target="_blank" rel="noopener noreferrer">plugin:kdu/recommended</a>'
  }
  Kdu.component('sg-enforcement', {
    template: '\
      <span>\
        <strong>Enforcement</strong>:\
        <span class="style-rule-tag" k-html="humanType"/>\
      </span>\
    ',
    props: {
      type: {
